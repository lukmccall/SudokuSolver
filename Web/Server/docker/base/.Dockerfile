...
# adding manager user
RUN mkdir -p conf/Catalina/localhost && echo '<Context privileged="true" antiResourceLocking="false" docBase="${catalina.home}/webapps/manager">\n<Valve className="org.apache.catalina.valves.RemoteAddrValve" allow="^.*$" /></Context>' > conf/Catalina/localhost/manager.xml && \
